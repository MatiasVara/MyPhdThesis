...
The specification contains a \emph{Flow} that define which operator is applied and on which models.\todo{The first flow (Listing~\ref{lst:bflowcamerasystem}: line 8) defines that the operator SyncFSMEventsAndActions is applied between the activity BatteryControl and the TFSM CameraControl. This results in the synchronization of the corresponding Action and FSMEvent by relying on theirs names.}
