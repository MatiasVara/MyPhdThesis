...
The resulting model of coordination is thus composed of six relations, \ie two \ccsl relations and four \moccml relations. In the modeling workbench, we use the \bflow specification together with the Gemoc Coordinated Execution Engine to execute the coordinated system. \todo{the picture shows the 4 models and the four gemoc execution engine and the gemoc coordinated engine. From the view it is possible to drive the simulation together with the corresponding animation} 

\todo{To show the studio}
...
\todo{In this example, we figured out that the state-space graph of the activity \emph{BatterySensor} is not finite. This makes that the state-space graph of the coordinated system is not computable. Thus, we could not get results on the scheduling state-space of the coordinated system.}
