...
To illustrate the use of event variables, we suppose the situation in which the coordination between events relies on a global synchronization clock. This is often in synchronous digital systems in which a clock signal is used to coordinate the actions in a circuit. Thus, we slightly modify the running example operator by considering the case in which the coordination between instances of \dse \emph{occurs} and instances of \dse \emph{executeIt} relies on a global clock signal. The selected instances of \dse \emph{occurs} (called \emph{sampledEvent}) are sampled by a global clock (called \emph{trigger}). This results in a new event which contains the occurrences of the \emph{sampledEvent} that were sampled by the \emph{trigger}. This new event ticks always in coincidence with the global clock and represents the last occurrence of the \emph{sampledEvent} that was sampled by the \emph{trigger}. 
	
\begin{lstlisting}[language=bcool,
caption={\bcool specification of the modified running example operator between the TFSM and Activity languages by using Event Variables},
...
In this subsection, we have presented the abstract syntax of \bcool. However, event expressions and relations are defined in dedicated libraries that must be imported (\emph{ImportedLibStatement} in Figure~\ref{fig:bcool}). For instance, in the previous examples, we relied on the library \emph{facilities.moccml} (Listing~\ref{lst:bcoolrunningexampletimed}: line 2) that contains the event expressions and relations that we used in the previous examples. We further explain the notion of library in the following subsection.
